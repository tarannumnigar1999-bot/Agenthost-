...
export default async function handler(req) {
  ...
  try {
    ...
    const planRes = await supabaseQuery(`users?id=eq.${authUser.id}&select=plan`, process.env.SUPABASE_SERVICE_KEY);
    ...
    // 5. Fetch the agent (must belong to this user)
    ...
    // 9. Response — matches what sendMessage() in the dashboard expects
    return json({ reply });
